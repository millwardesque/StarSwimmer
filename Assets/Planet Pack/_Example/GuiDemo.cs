...
public class GuiDemo : MonoBehaviour {
	...
	void Start(){
		...
		Color col = atmosphere.renderer.material.GetColor("_AtmoColor");
		r = (int)(col.r * 255);
		g = (int)(col.g * 255);
		b = (int)(col.b * 255);
		...
		atmosSize = atmosphere.renderer.material.GetFloat("_Size");
	}
	...
	void OnGUI(){
		...
		planetRotate.renderer.material.SetTexture("_MainTex",diffuse[index]);

		if (bumped){
			planetRotate.renderer.material.SetTexture("_Normals",normal[index]);
		}
		else{
			planetRotate.renderer.material.SetTexture("_Normals",noNormal);
		}
		...
		atmosphere.renderer.material.SetColor("_AtmoColor", new Color( (float)r/255f,(float)g/255f,(float)b/255f ));
		...
		atmosphere.renderer.material.SetFloat("_Size",atmosSize);

		planetRotate.renderer.material.SetColor("_AtmosNear", new Color( (float)r/255f,(float)g/255f,(float)b/255f ));
